	...
	if (loaded) {
		return ( 
			<div className="flex items-center justify-center flex-col">
				...
				<table className="table-auto m-5"><tbody>
					{rrpps.map((u,index) => {
						return (
							<tr key={u._id}>
								<td>{u.nombre} {u.apellido}</td>
								<td>
									<input className="rounded-lg border-solid border-2 border-sky-200 m-1 text-center w-12"
				...
				<div className="flex justify-between w-4/12">
					...
					<button className="border-solid border-2 border-green-100 bg-green-300 rounded px-2 py-1"
									onClick={e=>submitFreePasses(e)}>Guardar</button>
				</div>
			</div>
		);
	} else {
